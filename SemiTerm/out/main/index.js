...
function createWindow() {
  ...
  mainWindow.webContents.setWindowOpenHandler((details) => {
    require$$1$2.shell.openExternal(details.url);
    return { action: "deny" };
  });
...
require$$1$2.app.whenReady().then(() => {
  utils$1.electronApp.setAppUserModelId("com.electron.semiterm");
  require$$1$2.app.on("browser-window-created", (_, window2) => {
  ...
  require$$1$2.ipcMain.handle("db:get-connections", () => {
    return store.get("connections", []);
  });
  require$$1$2.ipcMain.handle("db:save-connection", (_event, connection) => {
    ...
    if (connection.id) {
      ...
      const newConnection = { ...connection, id: v4(), createdAt: now, updatedAt: now };
      connections.push(newConnection);
    }
    store.set("connections", connections);
    return connections;
  });
  require$$1$2.ipcMain.handle("db:delete-connection", (_event, id2) => {
    let connections = store.get("connections", []);
    connections = connections.filter((c) => c.id !== id2);
    store.set("connections", connections);
    return connections;
  });
  require$$1$2.ipcMain.on("ssh:connect", (_event, connection) => {
    console.log("SSH Connect:", connection.title);
  });
  require$$1$2.ipcMain.on("ssh:write", (_event, id2, data) => {
    console.log(`SSH Write to ${id2}:`, data);
  });
  require$$1$2.ipcMain.on("ssh:resize", (_event, id2, size) => {
    console.log(`SSH Resize for ${id2}:`, size);
  });
  require$$1$2.ipcMain.on("ssh:close", (_event, id2) => {
    console.log(`SSH Close:`, id2);
  });
